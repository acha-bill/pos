...
import {
  ...
  COLORS,
} from "../../components/primaryPieChart/primaryPieChart";
...
const Dashboard = () => {
  ...
  const getSales = async () => {
    const res = await apis.reportApi.sales(
      startDate.getTime(),
      endDate.getTime(),
      "",
      "",
  ...
  const getSelling = async () => {
    const res = await apis.reportApi.selling(
      startDate.getTime(),
      endDate.getTime()
    );
    ...
    if (
      worstSellingData.filter((i) => i.value === 0).length ===
      worstSellingData.length
    ) {
    ...
    if (
      topSellingData.filter((i) => i.value === 0).length ===
    ...
    <div>
      <div>
        <div className="overview">
          ...
          <div className="overview__inputs">
            <div>
              ...
              <span className="ml-2" onClick={() => setDatePickerOpen(true)}>
        ...
        <div className="row row-cols-4 d-flex justify-content-center align-items-center">
          <div
            className="col p-0 card mx-4 text-white bg-primary shadow"
          ...
          <div
            className="col p-0 card mx-4 text-white bg-success shadow"
            style={{ maxWidth: "15rem" }}
          >
          ...
          <div class="col">
            ...
            <PieChart width={500} height={300}>
              <Pie
                ...
                dataKey="value"
              >
                {worstSellingData.map((entry, index) => (
                  <Cell
                    key={`cell-${index}`}
                    fill={COLORS[index % COLORS.length]}
                  />
