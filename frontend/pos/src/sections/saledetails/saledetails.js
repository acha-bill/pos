...
const SaleDetailsReport = (props) => {
  ...
  const handleSearch = (event) => {
    ...
    let filtered = itemsData.filter(
      (li) => li.item.name.toLowerCase().indexOf(key) >= 0
    );
        ...
        <div>
          ...
          <div id="print">
            <div className="text-center mb-2">
              ...
              <span>
                Sales details report: {startDate.toLocaleDateString()} -{" "}
      ...
      <ReactTable
        ...
        columns={[
          ...
          },
          {
            ...
            Cell: (row) => {
              return (
                <div>{row.original.isWholeSale ? "whole sale" : "retail"}</div>
              );
            },
          },
          {
          ...
          },
          {
            ...
            Cell: (row) => {
              return (
                <div>{row.original.qty * row.original.item.purchasePrice}</div>
              );
            },
          },
          {
            ...
            Cell: (row) => {
              return (
                <div>
                  {row.original.total -
                    row.original.qty * row.original.item.purchasePrice}
                </div>
              );
            },
          },
